...
Unlike `python3 -m http.server` it (1) sends `Cache-Control: no-store` on every response, so the browser never
caches — each load re-fetches the wasm / hosts / js fresh, a hard refresh every time — and (2) opens a browser
tab automatically.
